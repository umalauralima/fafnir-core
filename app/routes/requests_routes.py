...
from app.dto.requests_dto import RequestCreateDTO, RequestDetailPublicDTO, RequestListResponseDTO
...
def create():
    ...
    return RequestDetailPublicDTO.model_validate(req).model_dump(), 201

@requests_bp.route("/requests/<int:id>", methods=["GET"])
@requires_permission("get_req")
def get_item(id):
    
    request = service.get_by_id(id)
    
    #TODO Diferenciar pela visao do usuario
    return RequestDetailPublicDTO.model_validate(request).model_dump(), 201